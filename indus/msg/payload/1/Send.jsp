<%@ page import="java.io.File,java.sql.Connection,java.sql.SQLException,com.cl.sql.PoolManager,com.cl.msg.*,java.math.*,java.util.*,org.apache.log4j.*,com.cl.msg.list.*"%><!--  errorPage="/msg/MSGErrorPage.jsp"%> -->
...
<%
		...
		try
		{
			...
			if(reply)
			{
				...
				} //end of if(receiverID==0 || receiverDestID==0)
				else
				{
				...
				outer:
				{
					...
					while(tokenizer.hasMoreElements())
					{
						...
						if(tokens.hasMoreElements())
						{
							if(tmpAgent==0)
								if(error==null)
									error = "<font class=\"new\">Sorry!! Message cannot be sent. Please check your receiver(s) list for the following error(s)-</font><br>" + tokens.nextToken();
								else 
									error += ", " + tokens.nextToken();
								//else   rList+= tokens.nextToken()+"<br>";
						}
	...
	<tr><td bgcolor="B4BC6D">
		...
		<tr><td bgcolor="B4BC6D" height="32" width="10">
			...
			<td><font class="cntblu">
				<a href="/" class="lnkblu">Home</a><font class="cntblu"> &gt; </font><%=cat==1?"<a href=\"/clzone/login/CLZone.jsp\" class=\"lnkblu\">CL Zone Home</a>":"<a href=\"/sis/sis.jsp\" class=\"lnkblu\">SIS page</a>"%> &gt;  Message Sent</font>
			</td>
		</tr>
		</table>
	</td></tr></table>
...
<%
		...
		attchFolderPath = application.getRealPath("/msg/attch") + File.separator 
							+ agent.getAgentID() + "_" + agent.getDestID() + File.separator;
		...
		attchFolderPath = application.getRealPath("/msg/attch") + File.separator + agent.getAgentID() 
							+ "_" + agent.getDestID() + File.separator;
		...
		String attchFileName = "";
		try
		{
			attchFileName = wpayload.getFilename();
			if(fwd && attchFileName.length() > 0)
			{
				wpayload.getFileFromBlob(attchFolderPath);
			}
		}
		...
		if(error==null)
		{
			if(StudentSend==-1)
			{
				msg.send(conn,wpayload,attchFolderPath,attchFileName,saveToSentItems);
			}
			else
			{
				try
				{
					sendResult = msgStudent.send(conn,wpayload,attchFolderPath,wpayload.getFilename(), privilege, ids,saveToSentItems);
	  ...
	  <tr> 
		...
		<%=	msg.getMsgID()>0 ? "Your Message has been sent successfully."+ sendResult 
			:(msg.getMsgID() == -1 
				...
				:"Your message cannot be sent due to some problem" + msg.getMsgID()) %></font></td>
	  </tr>
